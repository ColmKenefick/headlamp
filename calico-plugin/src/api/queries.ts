    ...
    queryFn: async () => {
      ...
      if (token) {
        ...
        return fetch('http://127.0.0.1:4466/externalproxy?clusterId=calico-hackathon', {
          method: 'GET',
          headers: {
            'Forward-to': 'https://p95znudz-multi-09-management.dev.calicocloud.io/tigera-elasticsearch/flows/statistics?type=PacketCount&groupBy=Policy&startTimeGt=-900&startTimeLt=-0',
            'Accept': '*/*',
            'Authorization': token,
          },
        }).then(res => res.json());
